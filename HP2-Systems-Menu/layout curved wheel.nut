...
class UserConfig {
	...
	</ label="Enble Scanlines", help="Show scanline effect", options="None,Light,Medium,Dark, RGB Effect", order=43 /> enable_scanline="";
...
if ( wheel_fade_ms == 0 ) {
	gameListBoxBackgroundAnimX.to = (flw + flx - crw - lbw)*1.13
...
function on_transition( ttype, var, ttime ) {
	if( ttype == Transition.ToNewSelection) {
			...
			gameListBoxBackgroundAnimX.to = (flw + flx - crw - lbw)*1.35
			if ( glist_delay != 0 ) gameListBoxBackgroundAnimX.hide( flw + flx - crw, fe.layout.time )
			gameListBoxBackgroundAnimA.to = ListBoxAlpha
			if ( glist_delay != 0 ) gameListBoxBackgroundAnimA.hide( 0, fe.layout.time )
...
local wheel_y = [ -fly*0.22, -fly*0.105, fly*0.0, fly*0.105, fly*0.215, fly*0.325, fly*0.455, fly*0.61, fly*0.72 fly*0.83, fly*0.935, fly*0.99, ];
...
local wheel_h = [  flh*0.10,  flh*0.10,  flh*0.10,  flh*0.10,  flh*0.10,  flh*0.10, flh*0.10,  flh*0.10,  flh*0.10,  flh*0.10,  flh*0.10,  flh*0.10, ];
...
mfliter2 = fe.add_text( "Curved Wheel View", flx*0.007, fly*0.04, flx*0.3, flh*0.015 ),
}
