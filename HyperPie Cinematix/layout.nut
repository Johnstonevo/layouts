...
class UserConfig {
   ...
   </ label="Select background image", help="Select theme background or flyer", options="default,System Specific,flyer,none", order=3 /> enable_bg="none";   
...
// Game info text.
if ( my_config["enable_ginfo"] == "Style1" )
{
 local titleText = fe.add_text( "   [DisplayName] > [Title] [Year] [Manufacturer] [Players]", 0, fly*0.97, flw*0.9, flh*0.022  );
 titleText.alpha = 200;
 }
if ( my_config["enable_ginfo"] == "Style2" )
{ 
  local titleText = fe.add_text( "[DisplayName] > [FilterName] > [Title] © [Year] [Manufacturer]     Played: [PlayedCount]   [Players]", 242, fly*0.97, flw*0.9, flh*0.022  );
 titleText.alpha = 200;
