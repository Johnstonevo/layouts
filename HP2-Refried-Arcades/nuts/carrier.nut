...
FUNCTIONS:    
    ...
    carrier.set_selector_color( 200, 0, 100 ) - sets the color of the selector rectangle if no file was specified
    
    carrier.set_background_color( 100, 0, 200 )  - sets the color of the Carrier surface background if no file was specified
...
class Carrier {
    ...
    constructor( carrierPosX, carrierPosY, carrierWidth, carrierHeight, tilesCount, _tilesOffscreen, _tilePadding, ... ) {
        ...
        if ( fileBackground != null ) surfaceBackground = fe.add_image( fileBackground, carrierPosX, 0, carrierWidth, carrierHeight )
        else {
            surfaceBackground = fe.add_text( "", carrierPosX, 0, carrierWidth, carrierHeight )
            surfaceBackground.set_bg_rgb( 50, 50, 50 )
        }

        if( fileSelector != null ) {
            //selector = fe.add_image( fileSelector, carrierPosX, 0, tileWidth + 2, tileHeight + 2) //TEMP +2
            selector = fe.add_image( fileSelector, carrierPosX, 0, tileWidth, tileHeight )
            //selector.origin_x = 1 //TEMP
            //selector.origin_y = 1 //TEMP
            selector.alpha = 255
        }
        else {
            selector = fe.add_text( "", carrierPosX, 0, tileWidth, tileHeight)
            selector.set_bg_rgb( 200, 200, 200 )
        }

        local index = -floor( tilesTotal / 2 )
        for( local i = 0; i < tilesTotal; i++ ) {
            local obj = fe.add_artwork( "boxart" )
            //obj.origin_x = 1 //TEMP
            //obj.origin_y = 1 //TEMP
            if( vertical ) {
                tilesTablePos.push( tileWidth * i + ( tilePadding / 2 ) )
                //obj.set_pos( carrierPosX + tilePadding, 0, tileWidth - tilePadding * 2 + 2, tileHeight - tilePadding * 2 + 2) //TEMP +2
                obj.set_pos( carrierPosX + tilePadding, 0, tileWidth - tilePadding * 2, tileHeight - tilePadding * 2 )
            } else {
                tilesTablePos.push( tileHeight * i + ( tilePadding / 2 ) )
                //obj.set_pos(carrierPosX, tilePadding, tileWidth - tilePadding * 2 + 2, tileHeight - tilePadding * 2 + 2 ) //TEMP +2
                obj.set_pos(carrierPosX, tilePadding, tileWidth - tilePadding * 2, tileHeight - tilePadding * 2 )
            }
    ...
    {
        if( fileBackground != null) surfaceBackground.set_rgb( r, g, b )
